...
export const translations = {
  ...
  zh: {
    ...
    editor_desc: "我们正在打造强大的工具，帮助你编辑和重组图片。",
    prompt: "提示词",
    optimize: "优化",
    optimizing: "优化中...",
    ...
    // Errors
    error_quota_exhausted: "你的今日配额已用完。你可以设置 Hugging Face 令牌以获取更多配额。",
    ...
    error_api_connection: "API 连接失败。请检查你的网络或令牌。",
    ...
    // FAQ
    faq_q1: "这个服务是免费的吗？",
    faq_a1: "是的，本项目完全免费。默认使用公共 API 配额。由于公共配额在高峰期可能受限，你可以在设置中配置自己的 Hugging Face 令牌，以获得更稳定的生成体验和更高的使用配额。Gitee AI 和 Model Scope 需要你提供令牌以使用其免费配额。",
    faq_q2: "我的隐私安全吗？",
    faq_a2: "绝对安全。我们非常重视隐私。所有的生成历史、设置和 Token 都仅存储在你的本地浏览器中 (LocalStorage)。我们没有后台数据库，也不会收集你的个人使用数据。温馨提示：生成的图片记录只会保留 24 小时，喜欢的图片请记得及时下载保存。提示词记录在你关闭页面后就会自动清空。",
    faq_q3: "多令牌系统是如何工作的？",
    faq_a3: "支持输入多个用逗号分隔的令牌。系统会自动建立令牌池。如果当前使用的 Token 耗尽了当日配额，系统会自动将其标记为当日耗尽，并无缝切换到下一个可用的 Token，确保你的创作不中断。该机制适用于 Hugging Face、Gitee AI 和 Model Scope。",
    faq_q4: "使用了哪些服务？",
    ...
    faq_a4_ms_post: "提供。",
    faq_q5: "我可以私有部署吗？",
    faq_a5: "可以！这是一个基于 MIT 协议的开源项目。你可以从 GitHub Fork 代码库，并将其自由部署到 Vercel、Cloudflare Pages 或任何静态托管平台。",
    faq_q6: "自动翻译是如何工作的？",
    faq_a6: "某些高级模型（如 FLUX 系列）对英文提示词的理解能力最佳。当选择 these models 时，“自动翻译”功能会默认开启。我们会检测你的提示词是否为非英文，并使用快速 LLM 自动将其翻译为英文，以确保生成最高质量的图片。",
    faq_q7: "提示词优化是如何工作的？",
    faq_a7: "我们使用强大的 LLM 将你简短的想法扩展为详细、富有艺术感的提示词。根据服务商的不同，我们会调用 DeepSeek 或 OpenAI 等模型。你可以在“设置”菜单的“提示词”选项卡中自定义“角色设定”，以控制 AI 重写提示词的方式。",
    faq_q8: "Live 图生成原理是什么？",
    faq_a8: "Live 功能利用先进的图生视频模型（如 Wan2.2），将静态图片转化为动态短视频。AI 会分析画面构图，生成自然的运动轨迹和电影级特效，赋予你的作品生命力。目前仅 Hugging Face 与 Gitee AI 支持。可以在设置页面滑动 Tab 栏查看更多设置。",
    faq_q9: "图片编辑功能是如何工作的？",
    faq_a9: "图片编辑器允许你利用 AI 修改现有图片。你可以在透明图层上绘画（使用画笔或矩形工具）来标注想要修改的区域，然后输入 AI 指令。系统会将你的涂鸦与原图合并，并发送给 Qwen-Image-Edit 模型进行处理。你还可以上传最多 3 张参考图，以引导 AI 的艺术风格或内容生成。",
    faq_q10: "云存储服务是如何工作的？",
    faq_a10: "我们支持 S3 兼容的存储服务（如 AWS S3, Cloudflare R2, MinIO）以及 WebDAV。你可以在“设置 > 存储服务”中配置你的存储凭证。配置完成后，你可以将生成的图片或视频直接上传到你的私有云端。你的凭证仅保存在本地浏览器中，绝不会上传到我们的服务器。",
    faq_q11: "什么是 OPFS 存储？",
    faq_a11: "OPFS (源私有文件系统) 是浏览器提供的高性能本地存储空间。它允许应用将生成的图片和视频直接保存到你设备的虚拟驱动器中。存储在这里的数据是私有的，完全保留在你的设备上，绝不会上传到我们的服务器。请注意：清除浏览器的站点数据或缓存将会永久删除这些文件。",
    ...
    gallery_setup_desc: "连接你的 S3 或 WebDAV 存储，随时随地查看你的创作。",
