        ...
        <div className="flex items-center justify-between text-sm group">
            ...
            <div className="flex gap-1 items-center">
                {keys.map((k, i) => (
                    ...
                    ? <span key={i} className="px-1.5 py-0.5 rounded bg-slate-900/10 dark:bg-white/10 font-mono text-xs">{k}</span>
                    : <span key={i} className="text-slate-500 dark:text-white/40 text-xs">{k}</span>
                ))}
            </div>
        </div>
    );
        ...
        <div className="w-full h-full flex flex-grow flex-col md:max-w-7xl md:mx-auto relative">
            ...
            >
                {isGenerating && (
                    <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white/80 dark:bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
                        ...
                        <p className="mt-8 text-slate-800 dark:text-white/80 font-medium animate-pulse text-lg">{t.dreaming}</p>
                        ...
                        <div className="w-full max-w-lg space-y-4">
                            <label
                                className={`cursor-pointer group flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-xl transition-all duration-300 animate-in zoom-in-95 ${
                                    isDragOver ? 'border-purple-500 bg-purple-500/10 scale-105' : 'border-slate-900/20 dark:border-white/20 bg-slate-900/[0.02] dark:bg-white/[0.02] hover:bg-slate-900/[0.05] dark:hover:bg-white/[0.05] hover:border-purple-500/50'
                                }`}
                            ...
                            >
                                ...
                                <div className="mb-6 p-5 rounded-full bg-slate-900/5 dark:bg-white/5 group-hover:bg-purple-500/20 group-hover:scale-110 transition-all duration-300 shadow-lg">
                                    <Upload className={`w-10 h-10 transition-colors ${isDragOver ? 'text-purple-400' : 'text-slate-500 dark:text-white/40 group-hover:text-purple-400'}`} />
                                </div>
                                <p className="text-slate-600 dark:text-white/60 font-medium text-lg group-hover:text-slate-900 dark:group-hover:text-white/90 transition-colors">{t.upload_image_cta}</p>
                            </label>
                            <div className="flex gap-4">
                                <button onClick={() => setShowHistoryModal(true)} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-slate-900/10 dark:bg-white/10 hover:bg-slate-900/15 dark:hover:bg-white/15 text-slate-800 dark:text-white/80 hover:text-slate-900 dark:hover:text-white border border-slate-900/10 dark:border-white/10 rounded-xl transition-all shadow-lg active:scale-95">
                                    <History className="w-4 h-4" /><span className="font-medium text-sm">{t.select_from_history}</span>
                                </button>
                                {isStorageEnabled && (
                                    <button onClick={() => setShowGalleryModal(true)} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-slate-900/10 dark:bg-white/10 hover:bg-slate-900/15 dark:hover:bg-white/15 text-slate-800 dark:text-white/80 hover:text-slate-900 dark:hover:text-white border border-slate-900/10 dark:border-white/10 rounded-xl transition-all shadow-lg active:scale-95">
                ...
                {generatedResult && image && (
                    <div className="absolute inset-0 z-50 bg-slate-50 dark:bg-[#0D0B14] animate-in fade-in duration-300">
                        <div className="relative w-full h-full overflow-hidden">
                             ...
                             <div className="absolute bottom-6 inset-x-0 flex justify-center pointer-events-none z-40">
                                <div className="pointer-events-auto max-w-[90%] overflow-x-auto scrollbar-hide rounded-2xl bg-white/80 dark:bg-black/60 backdrop-blur-md border border-slate-900/10 dark:border-white/10 shadow-2xl animate-in slide-in-from-bottom-4 duration-300">
                                    <div className="flex items-center gap-1 p-1.5 min-w-max">
                                        <Tooltip content={t.re_edit}>
                                            <button onClick={() => setGeneratedResult(null)} className="flex items-center justify-center w-10 h-10 rounded-xl text-slate-700 dark:text-white/70 hover:text-purple-400 hover:bg-slate-900/10 dark:hover:bg-white/10 transition-all"><RotateCcw className="w-5 h-5" /></button>
                                        </Tooltip>
                                        <div className="w-px h-5 bg-slate-900/10 dark:bg-white/10 mx-1"></div>
                                        {isStorageEnabled && provider !== 'modelscope' && (
                                            <>
                                                <Tooltip content={isUploading ? t.uploading : t.upload}>
                                                    <button onClick={() => onCloudUpload(isSourceNSFW)} disabled={isUploading} className="flex items-center justify-center w-10 h-10 rounded-xl text-slate-700 dark:text-white/70 hover:text-green-400 hover:bg-slate-900/10 dark:hover:bg-white/10 transition-all">
                                        ...
                                        <Tooltip content={t.menu_download}>
                                            <button onClick={() => handleDownloadResult(generatedResult, isSourceNSFW)} disabled={isDownloading} className="flex items-center justify-center w-10 h-10 rounded-xl text-slate-700 dark:text-white/70 hover:text-blue-400 hover:bg-slate-900/10 dark:hover:bg-white/10 transition-all">
                                                {isDownloading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Download className="w-5 h-5" />}
                                            </button>
                                        </Tooltip>
                                        <div className="w-px h-5 bg-slate-900/10 dark:bg-white/10 mx-1"></div>
                                        <Tooltip content={t.menu_exit}>
                                            <button onClick={() => setShowExitDialog(true)} className="flex items-center justify-center w-10 h-10 rounded-xl text-slate-700 dark:text-white/70 hover:text-red-400 hover:bg-red-500/10 transition-all"><LogOut className="w-5 h-5" /></button>
                                        </Tooltip>
                                    </div>
                                </div>
                             </div>
                ...
                {contextMenu && (
                    <>
                        ...
                        <div className="fixed z-50 min-w-[160px] bg-white/80 dark:bg-black/60 backdrop-blur-xl border border-slate-900/20 dark:border-white/20 rounded-xl shadow-2xl p-1 animate-in fade-in zoom-in-95 duration-150" style={{ left: contextMenu.x, top: contextMenu.y }}>
                            <button onClick={() => { setContextMenu(null); fileInputRef.current?.click(); }} className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-900 dark:text-white hover:bg-slate-900/10 dark:hover:bg-white/10 rounded-lg transition-colors text-left group"><ImageIcon className="w-4 h-4 text-purple-400 group-hover:scale-110" />{t.menu_replace}</button>
                            <button onClick={centerView} className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-900 dark:text-white hover:bg-slate-900/10 dark:hover:bg-white/10 rounded-lg transition-colors text-left group"><AlignCenter className="w-4 h-4 text-blue-400 group-hover:scale-110" />{t.menu_center}</button>
                            <button onClick={handleDownloadExport} className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-900 dark:text-white hover:bg-slate-900/10 dark:hover:bg-white/10 rounded-lg transition-colors text-left group"><Download className="w-4 h-4 text-green-400 group-hover:scale-110" />{t.menu_download}</button>
                            <div className="h-px bg-slate-900/10 dark:bg-white/10 my-1 mx-1" />
                ...
                {image && (
                    <div className="absolute bottom-6 left-6 hidden md:flex items-center gap-1 bg-white/80 dark:bg-black/60 backdrop-blur-md border border-slate-900/10 dark:border-white/10 rounded-full text-slate-700 dark:text-white/70 shadow-lg z-20">
                        <Tooltip content={t.sc_zoom_out}><button onClick={zoomOut} className="p-2 hover:bg-slate-900/10 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white transition-colors rounded-s-full"><Minus className="w-4 h-4" /></button></Tooltip>
                        <Tooltip content={t.sc_reset_view}><button onClick={zoomReset} className="p-2 text-xs font-mono min-w-[3rem] text-center outline-none select-none hover:bg-slate-900/10 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white transition-colors">{Math.round(scale * 100)}%</button></Tooltip>
                        <Tooltip content={t.sc_zoom_in}><button onClick={zoomIn} className="p-2 hover:bg-slate-900/10 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white transition-colors rounded-e-full"><Plus className="w-4 h-4" /></button></Tooltip>
                    </div>
                )}

                <div className="absolute bottom-6 right-6 hidden md:block z-20">
                    <Tooltip content={t.shortcuts_title} position="left">
                        <button onClick={() => setShowShortcuts(true)} className="p-2 bg-white/80 dark:bg-black/60 backdrop-blur-md border border-slate-900/10 dark:border-white/10 rounded-full text-slate-500 dark:text-white/50 hover:text-slate-900 dark:hover:text-white hover:bg-slate-900/10 dark:hover:bg-white/10 transition-colors shadow-lg"><Keyboard className="w-5 h-5" /></button>
                    </Tooltip>
                </div>
            ...
            {showExitDialog && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white dark:bg-[#1A1625] border border-slate-900/10 dark:border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">{t.exit_dialog_title}</h3>
                        <p className="text-slate-600 dark:text-white/60 text-sm mb-6">{t.exit_dialog_desc}</p>
                        <div className="flex justify-end gap-3">
                            <button onClick={() => setShowExitDialog(false)} className="px-4 py-2 rounded-lg text-slate-600 dark:text-white/60 hover:text-slate-900 dark:hover:text-white hover:bg-slate-900/5 dark:hover:bg-white/5 transition-colors text-sm font-medium">{t.cancel}</button>
            ...
            {showHistoryModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setShowHistoryModal(false)}>
                    <div className="bg-white dark:bg-[#1A1625] border border-slate-900/10 dark:border-white/10 rounded-2xl p-0 max-w-3xl w-[90vw] h-[80vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-5 border-b border-slate-900/10 dark:border-white/10 bg-slate-900/5 dark:bg-white/5">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><History className="w-5 h-5 text-purple-400" />{t.history_modal_title}</h3>
                            <button onClick={() => setShowHistoryModal(false)} className="text-slate-500 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition-colors"><X className="w-5 h-5" /></button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-slate-50 dark:bg-[#0D0B14]">
                            {history.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-white/30 space-y-4"><Sparkles className="w-12 h-12 opacity-50" /><p>{t.no_history_images}</p></div> : 
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">{history.map((img) => (
                                    <button key={img.id} onClick={() => { setIsSourceNSFW(!!img.isBlurred); loadEditorImage(img.url); setShowHistoryModal(false); }} className="group relative aspect-square rounded-xl overflow-hidden border border-slate-900/10 dark:border-white/10 hover:border-purple-500 transition-all hover:ring-4 hover:ring-purple-500/20 focus:outline-none">
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-t from-white/95 dark:from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end p-3"><p className="text-xs text-slate-900 dark:text-white/90 line-clamp-2 text-left">{img.prompt}</p><div className="flex items-center gap-2 mt-1 text-[10px] text-slate-500 dark:text-white/50"><Clock className="w-3 h-3" /><span>{new Date(img.timestamp).toLocaleDateString()}</span></div></div>
                                    </button>
                                ))}</div>}
                        </div>
                    </div>
            ...
            {showGalleryModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setShowGalleryModal(false)}>
                    <div className="bg-white dark:bg-[#1A1625] border border-slate-900/10 dark:border-white/10 rounded-2xl p-0 max-w-3xl w-[90vw] h-[80vh] flex flex-col shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between p-5 border-b border-slate-900/10 dark:border-white/10 bg-slate-900/5 dark:bg-white/5">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><Cloud className="w-5 h-5 text-purple-400" />{t.gallery_modal_title}</h3>
                            <button onClick={() => setShowGalleryModal(false)} className="text-slate-500 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition-colors"><X className="w-5 h-5" /></button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-5 custom-scrollbar bg-slate-50 dark:bg-[#0D0B14]">
                            {galleryLoading ? <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-white/30 space-y-4"><Loader2 className="w-10 h-10 animate-spin" /></div> : galleryFiles.length === 0 ? <div className="flex flex-col items-center justify-center h-full text-slate-400 dark:text-white/30 space-y-4"><CloudUpload className="w-12 h-12 opacity-50" /><p>{t.no_gallery_images}</p></div> : 
                                <>
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">{galleryFiles.slice(0, galleryLimit).map((file) => (
                                    <button key={file.key} onClick={() => { setIsSourceNSFW(file.key.includes('.NSFW')); loadEditorImage(galleryLocalUrls[file.key] || file.url); setShowGalleryModal(false); }} className="group relative aspect-square rounded-xl overflow-hidden border border-slate-900/10 dark:border-white/10 hover:border-purple-500 transition-all hover:ring-4 hover:ring-purple-500/20 focus:outline-none bg-slate-900/5 dark:bg-white/5">
                                ...
                                {galleryLimit < galleryFiles.length && <div className="mt-6 flex justify-center"><button onClick={() => setGalleryLimit(prev => prev + 20)} className="px-4 py-2 bg-slate-900/10 dark:bg-white/10 hover:bg-slate-900/15 dark:hover:bg-white/15 text-slate-800 dark:text-white/80 rounded-lg text-sm font-medium transition-colors">{t.load_more}</button></div>}
                                </>}
                        </div>
                    </div>
                </div>
            ...
            {showShortcuts && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setShowShortcuts(false)}>
                    <div className="bg-white dark:bg-[#1A1625] border border-slate-900/10 dark:border-white/10 rounded-2xl p-4 max-w-xl w-full shadow-2xl animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-between mb-6 border-b border-slate-900/10 dark:border-white/10 pb-4">
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2"><Keyboard className="w-5 h-5 text-purple-400" />{t.shortcuts_title}</h3>
                            <button onClick={() => setShowShortcuts(false)} className="text-slate-500 dark:text-white/40 hover:text-slate-900 dark:hover:text-white transition-colors"><X className="w-5 h-5" /></button>
                        </div>
